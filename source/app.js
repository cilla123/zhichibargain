...
App({
  ...
  // 请求用户个人信息 登录状态请求小程序个人信息，若未登录请求微信个人信息
  ...
  _requestUserXcxInfo: function (options) {
    ...
    this.sendRequest({
      ...
      success: function (res) {
        ...
        typeof options.success === 'function' && options.success();  // 登录成功后 重新发送请求
      },
      fail: function (res) {
  ...
  onPageLoad: function (event) {
    ...
    // let pageData = this.globalData.app_data[pageInstance.page_router];
    // let eles = pageData.eles;
    // let new_data = {};

    // eles.forEach(function (el, index) {
    //   if(el.type === 'text' && index === 2) {
    //     let data = {
    //       type: el.type,
    //       style: el.style,
    //       content: el.content,
    //       customFeature: el.customFeature
    //     };
    //     data.style = JSON.stringify(data.style);
    //     data.style = data.style.substr(1, data.style.length-1).replace('\\"', '');

    //     new_data[el.type.replace('-', '_') + (index + 1)] = data;
    //   }
    // })
    // new_data.dataId = detail;
    // new_data.addShoppingCartShow = false;
    // new_data.addTostoreShoppingCartShow = false;
    // pageInstance.setData(new_data);
  ...
  pageDataInitial: function () {
    ...
    if(this.getAppId() === 'MkGWtKBK7W' && !pageInstance.pageLoaded){
      this._getPageData(pageInstance.page_router);
      return;
      // let pageData = this.globalData.appData[pageInstance.page_router];
      // if(!pageData){
      //   this._getAppData(this.pageDataInitial);
      //   return;
      // }

      // let eles = this.globalData.appData[pageInstance.page_router].eles;
      // for(let i = 0; i <= eles.length-1; i++){
      //   let el = eles[i];
      //   newdata[el.type+(i+1)] = this._transformElementData(el);      
      // }
  ...
  _getPageData: function(router){
    ...
    this.sendRequest({
      ...
      success: function(res){
        res.components = JSON.parse(JSON.stringify(res));
        currentpage.setData(res);
        currentpage.pageLoaded = true;
        that.pageDataInitial();
      }
    })
  },
  // _getAppData: function(callback){
  //   let that = this;
  //   this.sendRequest({
  //     hideLoading: true,
  //     url: '/index.php?r=AppData/detail',
  //     success: function(res){
  //       that._parseAppData(res.data);
   ...
   */
  // _parseAppData: function (data) {
  //   this.globalData.appData = JSON.parse(data.app_data);
  //   this.globalData.cataData = JSON.parse(data.cata_data);
  // },
  // _transformElementData: function(el){
  //   switch(el.type){
  //     case 'text': break;
  //   }
   ...
   * 滚动到达页面底部触发的函数。
   *
   * @param  {Array}     reachBottomFuc                需要触发的函数集合，由对象组成[{param: {}, triggerFuc : function}]
   *                     reachBottomFuc[i].triggerFuc  触发的函数
   *                     reachBottomFuc[i].param       这些参数会传入触发的函数中
   */
  ...
  _refreshFranchiseeList: function (targetCompId, requestData, pageInstance) {
    ...
    requestData.page = -1; // 暂时处理子店列表高度自适应无法加载第二页数据问题，等上拉加载数据做完之后可以删除
...
    return  s;
  ...
  /**
   * 返回登录标记。
   * 需要判断登录时调用 isLogin 方法。
   * @return {boolean} true:已登录过, false:未登录
   */
  getIsLogin: function () {
  ...
  globalData:{
    appId: 'HDp19SpV6L',
        tabBarPagePathArr: '["/pages/page10000/page10000","/pages/page10001/page10001","/pages/page10002/page10002","/pages/page10003/page10003"]',
     ...
     *
     * 包含字段:
     * @type {string}  add_time
     * @type {string}  app_id             小程序webapp应用ID
     * @type {string}  balance            账户余额
     * @type {string}  can_use_integral   可用积分
     * @type {string}  city               城市名称 例：'Shenzhen'
     * @type {string}  company            公司信息
     * @type {string}  contact_status     联系状态
     * @type {string}  country            国家名称 例：'China'
     * @type {string}  cover_thumb        微信头像
     * @type {string}  email              邮箱
     * @type {string}  extra_attr         未知字段，询问后台开发人员
     * @type {string}  from_manual        未知字段，询问后台开发人员
     * @type {string}  id                 未知字段，询问后台开发人员
     * @type {string}  integral           未知字段，询问后台开发人员
     * @type {string}  is_deleted         未知字段，询问后台开发人员
     * @type {string}  is_update          未知字段，询问后台开发人员
     * @type {string}  last_login_date    上次登录时间
     * @type {string}  last_time          未知字段，询问后台开发人员
     * @type {string}  model              未知字段，询问后台开发人员
     * @type {string}  nickname           微信昵称
     * @type {string}  phone              绑定手机号
     * @type {string}  province           省份名称 例：'Guangdong'
     * @type {string}  qq                 QQ号
     * @type {string}  remark             未知字段，询问后台开发人员
     * @type {string}  remark_email       未知字段，询问后台开发人员
     * @type {string}  remark_phone       未知字段，询问后台开发人员
     * @type {string}  remark_qq          未知字段，询问后台开发人员
     * @type {string}  remark_weixin_code 未知字段，询问后台开发人员
     * @type {string}  sex                性别 0=男，1=女
     * @type {string}  sns_banned_flag    未知字段，询问后台开发人员
     * @type {string}  sns_gagged_flag    未知字段，询问后台开发人员
     * @type {string}  sns_integral       未知字段，询问后台开发人员
     * @type {string}  status             未知字段，询问后台开发人员
     * @type {string}  subscribe          未知字段，询问后台开发人员
     * @type {string}  tags               未知字段，询问后台开发人员
     * @type {string}  terminal           未知字段，询问后台开发人员
     * @type {string}  today_share_count  未知字段，询问后台开发人员
     * @type {string}  total_share_count  未知字段，询问后台开发人员
     * @type {string}  type               未知字段，询问后台开发人员
     * @type {string}  update_time        未知字段，询问后台开发人员
     * @type {string}  user_token         用户标识
     * @type {string}  weixin_code        未知字段，询问后台开发人员
     * @type {string}  weixin_id          未知字段，询问后台开发人员
     */
    userInfo: {},
    /**
     * systemInfo 保存系统相关信息
     * @type {Object}  具体字段请参考小程序官方文档 https://mp.weixin.qq.com/debug/wxadoc/dev/api/systeminfo.html
     */
    systemInfo: null,
    sessionKey: '',
    ...
    cdnUrl: 'http://ceshi.zhichiwangluo.com',
    defaultPhoto: 'http://img.zhichiwangluo.com/zc_app_default_photo.png',
    siteBaseUrl:'https://ceshi.zhichiwangluo.com',
    appTitle: 'Apk',
    appDescription: '我的应用',
    appLogo: 'http://img.weiye.me/zcimgdir/album/file_5a321c98bc9d5.jpg'
  }
})
